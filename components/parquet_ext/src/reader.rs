...
impl Drop for ObjectStoreReader {
    fn drop(&mut self) {
        debug!("objectStoreReader dropped, path:{}, elapsed:{:?}",&self.path,self.begin.elapsed());
    }
}
...
impl AsyncFileReader for ObjectStoreReader {
    ...
    fn get_metadata(&mut self) -> BoxFuture<'_, parquet::errors::Result<Arc<parquet::file::metadata::ParquetMetaData>>> {
